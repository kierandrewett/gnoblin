...
struct ImagesDeleter {
    void operator()(XcursorImages *images) const {
        if (images) xcursor_images_destroy(images);
    }
};
}
...
XcursorImages *gnoblin_hyprcursor_load(const char *theme, const char *shape, int size) {
    if (g_strcmp0(g_getenv("GNOME_SHELL_SESSION_MODE"), "gnoblin") != 0 ||
        !shape || size < 1 || size > 1024)
        return nullptr;

    const char *override_theme = g_getenv("HYPRCURSOR_THEME");
    const std::string requested = override_theme && *override_theme ? override_theme : theme ? theme : "";
    ...
    try {
        ...
        if (!manager->valid()) return nullptr;

        const Hyprcursor::SCursorStyleInfo style{static_cast<unsigned int>(size)};
        if (!loaded_sizes.contains(size)) {
            if (!manager->loadThemeStyle(style)) return nullptr;
            loaded_sizes.insert(size);
        }
        const auto data = manager->getShape(shape, style);
        if (data.images.empty() || data.images.size() > 1024) return nullptr;

        std::unique_ptr<XcursorImages, ImagesDeleter> images(xcursor_images_create(data.images.size()));
        ...
        for (size_t i = 0; i < data.images.size(); i++) {
            ...
            if (width < 1 || height < 1 || width > 4096 || height > 4096 ||
                frame.hotspotX < 0 || frame.hotspotX >= width || frame.hotspotY < 0 || frame.hotspotY >= height)
                return nullptr;
            ...
            for (int row = 0; row < height; row++)
                std::memcpy(image->pixels + row * width, pixels + row * stride, width * sizeof(XcursorPixel));
        }
