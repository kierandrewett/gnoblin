...
/* Generated by scripts/stringify-file.py from src/data/gnoblin.defaults.conf;
 * provides `static const char GNOBLIN_DEFAULTS_CONF[]`. Compositor build only. */
...
void gnoblin_config_reload(void) {
     ...
     * Rust shell embeds). The user's file is layered on top below, so any key
     * they set wins. Only the compositor build defines this; the mutter overlay
     * compiles the same parser without the embedded header. */
    parse_into(table, GNOBLIN_DEFAULTS_CONF);
#endif
