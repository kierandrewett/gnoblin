...
static uint32_t current_protocol_mode(MetaWaylandOutputPowerContext* ctx) {
    MetaPowerSave mode = meta_monitor_manager_get_power_save_mode(ctx->monitor_manager);

    return mode == META_POWER_SAVE_ON ? ZWLR_OUTPUT_POWER_V1_MODE_ON
                                      : ZWLR_OUTPUT_POWER_V1_MODE_OFF;
}

static void output_power_send_mode(MetaWaylandOutputPower* power) {
    if (!power->monitor)
        return;

    zwlr_output_power_v1_send_mode(power->resource, current_protocol_mode(power->ctx));
}

static void on_power_save_changed(MetaMonitorManager* manager, MetaPowerSaveChangeReason reason,
                                  gpointer user_data) {
...
static void output_power_fail(MetaWaylandOutputPower* power) {
    if (!power->monitor)
        return;
...
static void on_monitors_changed(MetaMonitorManager* manager, gpointer user_data) {
    MetaWaylandOutputPowerContext* ctx = user_data;
...
static void output_power_destroy(struct wl_resource* resource) {
...
static void output_power_handle_set_mode(struct wl_client* client, struct wl_resource* resource,
                                         uint32_t mode) {
    MetaWaylandOutputPower* power = wl_resource_get_user_data(resource);
    ...
    target = mode == ZWLR_OUTPUT_POWER_V1_MODE_ON ? META_POWER_SAVE_ON : META_POWER_SAVE_OFF;

    /* mutter DPMS is global: apply to the whole backend (see file header). */
    if (klass->set_power_save_mode)
        klass->set_power_save_mode(manager, target);
    meta_monitor_manager_power_save_mode_changed(manager, target,
                                                 META_POWER_SAVE_CHANGE_REASON_MODE_CHANGE);
}

static void output_power_handle_destroy(struct wl_client* client, struct wl_resource* resource) {
...
static void manager_get_output_power(struct wl_client* client, struct wl_resource* manager_resource,
                                     uint32_t id, struct wl_resource* output_resource) {
    MetaWaylandOutputPowerContext* ctx = wl_resource_get_user_data(manager_resource);
    ...
    power->ctx = ctx;
    wl_resource_set_implementation(resource, &output_power_interface, power, output_power_destroy);
...
static void bind_output_power_manager(struct wl_client* client, void* data, uint32_t version,
                                      uint32_t id) {
    MetaWaylandOutputPowerContext* ctx = data;
    struct wl_resource* resource;

    resource = wl_resource_create(client, &zwlr_output_power_manager_v1_interface, version, id);
    if (!resource) {
        wl_client_post_no_memory(client);
        return;
    }
...
void meta_wayland_init_output_power_management(MetaWaylandCompositor* compositor) {
