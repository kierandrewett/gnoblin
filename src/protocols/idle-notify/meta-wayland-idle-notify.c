...
static void on_user_active(MetaIdleMonitor* monitor, guint watch_id, gpointer user_data) {
...
static void on_idle(MetaIdleMonitor* monitor, guint watch_id, gpointer user_data) {
    MetaWaylandIdleNotification* notification = user_data;
    ...
    if (notification->active_watch_id == 0) {
        notification->active_watch_id = meta_idle_monitor_add_user_active_watch(
            notification->monitor, on_user_active, notification, NULL);
    }
}
...
static void arm_idle_watch(MetaWaylandIdleNotification* notification) {
    ...
    notification->idle_watch_id = meta_idle_monitor_add_idle_watch_full(
        notification->monitor, interval, on_idle, notification, NULL, notification->flags);
}
...
static void idle_notification_destroy(struct wl_resource* resource) {
    ...
    if (notification->idle_watch_id != 0)
        meta_idle_monitor_remove_watch(notification->monitor, notification->idle_watch_id);
    if (notification->active_watch_id != 0)
        meta_idle_monitor_remove_watch(notification->monitor, notification->active_watch_id);

    g_free(notification);
}

static void idle_notification_handle_destroy(struct wl_client* client,
                                             struct wl_resource* resource) {
...
static void create_notification(struct wl_client* client, struct wl_resource* manager_resource,
                                uint32_t id, uint32_t timeout, struct wl_resource* seat_resource,
                                MetaIdleMonitorWatchFlags flags) {
    ...
    resource = wl_resource_create(client, &ext_idle_notification_v1_interface,
    ...
    wl_resource_set_implementation(resource, &notification_interface, notification,
                                   idle_notification_destroy);

    arm_idle_watch(notification);
}

static void notifier_get_idle_notification(struct wl_client* client, struct wl_resource* resource,
...
static void notifier_destroy(struct wl_client* client, struct wl_resource* resource) {
    wl_resource_destroy(resource);
}
...
static void bind_idle_notifier(struct wl_client* client, void* data, uint32_t version,
                               uint32_t id) {
    MetaWaylandCompositor* compositor = data;
    struct wl_resource* resource;

    resource = wl_resource_create(client, &ext_idle_notifier_v1_interface, version, id);
    if (!resource) {
        wl_client_post_no_memory(client);
        return;
    }

    wl_resource_set_implementation(resource, &notifier_interface, compositor, NULL);
}
