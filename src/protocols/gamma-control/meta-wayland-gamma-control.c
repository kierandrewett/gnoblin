...
static size_t monitor_gamma_size(MetaMonitor* monitor) {
...
static void gamma_control_restore(MetaWaylandGammaControl* control) {
    GList* l;
...
static void gamma_control_drop_saved(MetaWaylandGammaControl* control) {
    GList* l;
...
static void gamma_control_fail(MetaWaylandGammaControl* control) {
    if (!control->monitor)
        return;
...
static void gamma_control_handle_set_gamma(struct wl_client* client, struct wl_resource* resource,
                                           int32_t fd) {
    MetaWaylandGammaControl* control = wl_resource_get_user_data(resource);
...
static void gamma_control_handle_destroy(struct wl_client* client, struct wl_resource* resource) {
...
static void manager_get_gamma_control(struct wl_client* client,
    ...
    if (g_hash_table_contains(ctx->controls, monitor)) {
        MetaWaylandGammaControl* existing = g_hash_table_lookup(ctx->controls, monitor);
...
static void bind_gamma_control_manager(struct wl_client* client, void* data, uint32_t version,
                                       uint32_t id) {
    MetaWaylandGammaContext* ctx = data;
    struct wl_resource* resource;

    resource = wl_resource_create(client, &zwlr_gamma_control_manager_v1_interface, version, id);
    if (!resource) {
        wl_client_post_no_memory(client);
        return;
    }
...
void meta_wayland_init_gamma_control(MetaWaylandCompositor* compositor) {
