...
G_DECLARE_FINAL_TYPE(MetaDataControlSource, meta_data_control_source, META, DATA_CONTROL_SOURCE,
                     MetaSelectionSource)
...
static void meta_data_control_source_finalize(GObject* object) {
...
static void meta_data_control_source_read_async(MetaSelectionSource* source_base,
                                                const gchar* mimetype, GCancellable* cancellable,
    ...
    if (!g_unix_set_fd_nonblocking(pipe_fds[0], TRUE, NULL) ||
        ...
        g_task_report_new_error(source, callback, user_data, meta_data_control_source_read_async,
                                G_IO_ERROR, G_IO_ERROR_FAILED, "Could not make pipe nonblocking");
        close(pipe_fds[0]);
        close(pipe_fds[1]);
        return;
    }
...
static GInputStream* meta_data_control_source_read_finish(MetaSelectionSource* source,
...
static GList* meta_data_control_source_get_mimetypes(MetaSelectionSource* source_base) {
    MetaDataControlSource* source = META_DATA_CONTROL_SOURCE(source_base);

    return g_list_copy_deep(source->mimetypes, (GCopyFunc)g_strdup, NULL);
}

static void meta_data_control_source_deactivated(MetaSelectionSource* source_base) {
    MetaDataControlSource* source = META_DATA_CONTROL_SOURCE(source_base);
...
static void meta_data_control_source_class_init(MetaDataControlSourceClass* klass) {
...
static void meta_data_control_source_init(MetaDataControlSource* source) {}
...
static MetaSelection* data_control_selection(MetaWaylandDataControl* data_control) {
    MetaContext* context = meta_wayland_compositor_get_context(data_control->compositor);
...
static void data_control_source_offer(struct wl_client* client, struct wl_resource* resource,
                                      const char* mime_type) {
    MetaDataControlSource* source = wl_resource_get_user_data(resource);

    if (source->used) {
        wl_resource_post_error(resource, EXT_DATA_CONTROL_SOURCE_V1_ERROR_INVALID_OFFER,
...
static void data_control_source_handle_destroy(struct wl_client* client,
                                               struct wl_resource* resource) {
...
static void data_control_offer_transfer_cb(GObject* object, GAsyncResult* result,
                                           gpointer user_data) {
...
static void data_control_offer_receive(struct wl_client* client, struct wl_resource* resource,
    ...
    meta_selection_transfer_async(data_control_selection(offer->data_control),
                                  offer->selection_type, mime_type, -1, stream, NULL,
                                  data_control_offer_transfer_cb, NULL);
    g_object_unref(stream);
}

static void data_control_offer_handle_destroy(struct wl_client* client,
                                              struct wl_resource* resource) {
    wl_resource_destroy(resource);
}
...
static void device_advertise_selection(MetaWaylandDataControlDevice* device,
    ...
    offer_resource = wl_resource_create(client, &ext_data_control_offer_v1_interface,
                                        wl_resource_get_version(device->resource), 0);
    if (!offer_resource) {
        wl_client_post_no_memory(client);
        return;
    }
...
static void on_owner_changed(MetaSelection* selection, guint selection_type,
                             MetaSelectionSource* new_owner, gpointer user_data) {
    MetaWaylandDataControlDevice* device = user_data;

    if (selection_type != META_SELECTION_CLIPBOARD && selection_type != META_SELECTION_PRIMARY)
        return;
...
static void device_set_selection(MetaWaylandDataControlDevice* device, MetaSelectionType type,
    ...
    if (source_resource) {
        ...
        if (source->used) {
            wl_resource_post_error(device->resource, EXT_DATA_CONTROL_DEVICE_V1_ERROR_USED_SOURCE,
        ...
        *tracked = source;
        ...
        if (*tracked) {
            meta_selection_unset_owner(selection, type, META_SELECTION_SOURCE(*tracked));
            *tracked = NULL;
        }
    }
}

static void device_set_selection_request(struct wl_client* client, struct wl_resource* resource,
                                         struct wl_resource* source_resource) {
...
static void device_set_primary_selection_request(struct wl_client* client,
                                                 struct wl_resource* resource,
                                                 struct wl_resource* source_resource) {
    MetaWaylandDataControlDevice* device = wl_resource_get_user_data(resource);

    device_set_selection(device, META_SELECTION_PRIMARY, source_resource, &device->primary_source);
}

static void device_handle_destroy(struct wl_client* client, struct wl_resource* resource) {
    wl_resource_destroy(resource);
}
...
static void device_destroy(struct wl_resource* resource) {
    MetaWaylandDataControlDevice* device = wl_resource_get_user_data(resource);

    g_clear_signal_handler(&device->owner_changed_id, data_control_selection(device->data_control));
    device->data_control->devices = g_list_remove(device->data_control->devices, device);

    g_free(device);
}

static void manager_get_data_device(struct wl_client* client, struct wl_resource* manager_resource,
                                    uint32_t id, struct wl_resource* seat_resource) {
    MetaWaylandDataControl* data_control = wl_resource_get_user_data(manager_resource);
    ...
    wl_resource_set_implementation(resource, &device_interface, device, device_destroy);
...
static void bind_data_control_manager(struct wl_client* client, void* data, uint32_t version,
                                      uint32_t id) {
    MetaWaylandDataControl* data_control = data;
    struct wl_resource* resource;

    resource = wl_resource_create(client, &ext_data_control_manager_v1_interface, version, id);
    if (!resource) {
        wl_client_post_no_memory(client);
        return;
    }

    wl_resource_set_implementation(resource, &manager_interface, data_control, NULL);
}
