...
static void set_fades(struct wl_client* client, struct wl_resource* resource,
                      struct wl_resource* surface_resource, struct wl_array* fades) {
    ...
    gboolean valid =
        fades->size % (5 * sizeof(int32_t)) == 0 && fades->size <= MAX_FADES * 5 * sizeof(int32_t);

    for (size_t i = 0; valid && i < fades->size / sizeof(int32_t); i += 5) {
        for (size_t j = 0; j < 4; j++)
            valid = valid && values[i + j] >= -65536 * 256 && values[i + j] <= 65536 * 256;
        valid = valid && values[i + 2] >= 0 && values[i + 3] >= 0 && values[i + 4] >= 0 &&
                values[i + 4] <= 256;
    }
    if (!valid) {
    ...
    g_object_set_data_full(G_OBJECT(pending), FADE_KEY, g_bytes_new(fades->data, fades->size),
...
static void bind_manager(struct wl_client* client, void* data, uint32_t version, uint32_t id) {
    struct wl_resource* resource =
        wl_resource_create(client, &gnoblin_blur_fade_manager_v1_interface, 1, id);
...
void meta_wayland_blur_fade_merge_state(MetaWaylandSurfaceState* from,
...
void meta_wayland_blur_fade_apply_state(MetaWaylandSurface* surface,
    ...
    if (fades)
        g_object_set_data_full(G_OBJECT(surface), FADE_KEY, g_bytes_ref(fades),
                               (GDestroyNotify)g_bytes_unref);
    fades = g_object_get_data(G_OBJECT(surface), FADE_KEY);
    actor = CLUTTER_ACTOR(meta_wayland_surface_get_actor(surface));
    if (actor && fades)
        g_object_set_data_full(G_OBJECT(actor), FADE_KEY, g_bytes_ref(fades),
                               (GDestroyNotify)g_bytes_unref);
    window = meta_wayland_surface_get_window(surface);
    if (!window || !fades || g_object_get_data(G_OBJECT(window), FADE_KEY) == fades)
        return;
    g_object_set_data_full(G_OBJECT(window), FADE_KEY, g_bytes_ref(fades),
                           (GDestroyNotify)g_bytes_unref);
