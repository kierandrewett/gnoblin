...
static void
set_pending_region (MetaWaylandSurface *surface, MtkRegion *region)
{
...
static void
surface_destroyed (MetaWaylandSurface *surface, BackgroundSurface *effect)
{
  effect->surface = NULL;
}

static void
free_effect (struct wl_resource *resource)
{
...
static void
set_blur_region (struct wl_client *client, struct wl_resource *resource,
  ...
  if (!effect->surface)
    {
      wl_resource_post_error (resource,
        EXT_BACKGROUND_EFFECT_SURFACE_V1_ERROR_SURFACE_DESTROYED, "Surface destroyed");
      return;
    }
...
static void
get_background_effect (struct wl_client *client, struct wl_resource *resource,
                       uint32_t id, struct wl_resource *surface_resource)
{
  ...
  struct wl_resource *object = wl_resource_create (client,
  ...
  effect->destroy_handler = g_signal_connect (surface, "destroy",
    G_CALLBACK (surface_destroyed), effect);
...
bind_manager (struct wl_client *client, void *data, uint32_t version, uint32_t id)
{
  struct wl_resource *resource = wl_resource_create (client,
    &ext_background_effect_manager_v1_interface, 1, id);
  if (!resource)
    {
      wl_client_post_no_memory (client);
      return;
...
meta_wayland_init_background_effect (MetaWaylandCompositor *compositor)
{
  if (!gnoblin_config_protocol_enabled ("ext-background-effect-v1"))
    return;
  g_signal_new ("gnoblin-background-effect-changed", META_TYPE_DISPLAY, G_SIGNAL_RUN_LAST,
                0, NULL, NULL, NULL, G_TYPE_NONE, 1, CLUTTER_TYPE_ACTOR);
  if (!wl_global_create (compositor->wayland_display,
                        &ext_background_effect_manager_v1_interface, 1, NULL, bind_manager))
    g_error ("Failed to register ext-background-effect-v1");
}

void
meta_wayland_background_effect_merge_state (MetaWaylandSurfaceState *from,
...
void
meta_wayland_background_effect_apply_state (MetaWaylandSurface *surface,
                                           MetaWaylandSurfaceState *state)
{
  MtkRegion *region = g_object_steal_data (G_OBJECT (state), REGION_KEY);
  if (region)
    g_object_set_data_full (G_OBJECT (surface), REGION_KEY, region, (GDestroyNotify) mtk_region_unref);
 ...
 */
MtkRegion *
meta_gnoblin_background_effect_get_region (ClutterActor *actor)
{
