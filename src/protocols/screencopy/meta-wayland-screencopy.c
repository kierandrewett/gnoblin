...
static void destroy_frame(struct wl_resource* resource) {
    MetaWaylandScreencopyFrame* frame = wl_resource_get_user_data(resource);

    g_free(frame);
}

static void send_failed(MetaWaylandScreencopyFrame* frame) {
    if (frame->resource)
...
static gboolean calculate_buffer_geometry(const MtkRectangle* rect, float scale, int* width,
    ...
    if (scaled_width <= 0.0 || scaled_width > G_MAXINT || scaled_height <= 0.0 ||
        scaled_height > G_MAXINT)
        return FALSE;
...
static gboolean validate_shm_buffer(MetaWaylandScreencopyFrame* frame,
...
static void copy_frame_to_buffer(struct wl_client* client, struct wl_resource* resource,
                                 struct wl_resource* buffer_resource, gboolean with_damage) {
    MetaWaylandScreencopyFrame* frame = wl_resource_get_user_data(resource);
    struct wl_shm_buffer* shm_buffer;
    MetaContext* context;
    MetaBackend* backend;
    ...
    if (frame->with_damage) {
        zwlr_screencopy_frame_v1_send_damage(resource, 0, 0, frame->buffer_width,
                                             frame->buffer_height);
    }
    ...
    zwlr_screencopy_frame_v1_send_ready(resource, (uint32_t)(((uint64_t)ts.tv_sec) >> 32),
...
static float scale_for_monitor(MetaBackend* backend, MetaLogicalMonitor* logical_monitor) {
...
static gboolean clip_region_to_monitor(const MtkRectangle* monitor_rect, int32_t x, int32_t y,
                                       int32_t width, int32_t height, MtkRectangle* clipped) {
    ...
    if (monitor_rect->width <= 0 || monitor_rect->height <= 0 || monitor_x2 > G_MAXINT ||
        monitor_y2 > G_MAXINT || width <= 0 || height <= 0)
        return FALSE;
...
static void create_frame(struct wl_client* client, struct wl_resource* manager_resource,
                         uint32_t frame_id, gboolean overlay_cursor,
                         struct wl_resource* output_resource, gboolean region, int32_t x, int32_t y,
    ...
    frame->resource =
        wl_resource_create(client, &zwlr_screencopy_frame_v1_interface, version, frame_id);
    if (!frame->resource) {
    ...
    if (region) {
        if (!clip_region_to_monitor(&monitor_rect, x, y, width, height, &frame->rect)) {
            send_failed(frame);
            g_steal_pointer(&frame);
            return;
        }
    ...
    if (!calculate_buffer_geometry(&frame->rect, frame->scale, &frame->buffer_width,
                                   &frame->buffer_height, &frame->buffer_stride)) {
        send_failed(frame);
        g_steal_pointer(&frame);
        return;
    }

    zwlr_screencopy_frame_v1_send_buffer(frame->resource, WL_SHM_FORMAT_ARGB8888,
...
static void manager_capture_output(struct wl_client* client, struct wl_resource* resource,
                                   uint32_t frame, int32_t overlay_cursor,
                                   struct wl_resource* output) {
    create_frame(client, resource, frame, overlay_cursor, output, FALSE, 0, 0, 0, 0);
}

static void manager_capture_output_region(struct wl_client* client, struct wl_resource* resource,
                                          uint32_t frame, int32_t overlay_cursor,
                                          struct wl_resource* output, int32_t x, int32_t y,
                                          int32_t width, int32_t height) {
    create_frame(client, resource, frame, overlay_cursor, output, TRUE, x, y, width, height);
}
...
static void bind_screencopy(struct wl_client* client, void* data, uint32_t version, uint32_t id) {
    ...
    wl_resource_set_implementation(resource, &manager_interface, compositor, NULL);
}
