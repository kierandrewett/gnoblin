...
export class LayerCompanions {
    ...
    track(actor) {
        ...
        const destroy = actor.connect("destroy", () => {
            ...
            for (const [parent, children] of this.saved)
                this.saved.set(
                    parent,
                    children.filter((child) => child !== actor),
                );
            this.apply();
        });
    ...
    dismiss(surface, done) {
        ...
        for (const actor of global.get_window_actors()) {
            ...
            if (!actor.visible || !window || !request.companions.includes(Meta.gnoblin_layer_namespace(window)))
                continue;
            ...
            const offset =
                frame.y < monitor.y + monitor.height / 2
                    ? monitor.y - frame.y - frame.height
                    : monitor.y + monitor.height - frame.y;
    ...
    apply() {
        ...
        for (const request of this.requests) {
            for (const { actor: overlay } of byNamespace.get(request.surface) ?? []) {
                ...
                if (!parent) continue;
                const companions = (byParent.get(parent) ?? [])
                    .filter(
                        (entry) =>
                            entry.actor.meta_window.get_monitor() === overlay.meta_window.get_monitor() &&
                            request.companions.includes(entry.namespace),
                    )
