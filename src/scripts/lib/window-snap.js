...
export class WindowSnap {
    ...
    begin(window) {
        ...
        if (
            ...
            !window.allows_move() ||
            (!window.allows_resize() && !window.get_maximize_flags()) ||
        ...
        if (previous) {
            ...
            window.move_resize_frame(
                true,
                Math.round(x - previous.width * ratio),
                Math.round(y - Math.min(32, y - frame.y)),
                previous.width,
                previous.height,
            );
    ...
    publish(client = null) {
        ...
        for (const peer of this.bridge.clients) if (peer.trackSnap) this.bridge.send(peer, state);
    }

    hasSubscribers() {
        for (const client of this.bridge.clients) if (client.trackSnap) return true;
        return false;
    }
        ...
        for (const region of record.regions) {
            for (const rect of [region.hit, region.target]) {
                if (
                    ...
                    Math.abs(rect.x) > 100000 ||
                    Math.abs(rect.y) > 100000 ||
                    rect.width < 1 ||
                    rect.height < 1 ||
                    rect.width > 32768 ||
                    rect.height > 32768
    ...
    apply(window, target, monitorId, original = null, maximize = false) {
        if (
            ...
            throw new Error("window cannot snap");
        ...
        if (
            ...
            target.x < area.x ||
            target.y < area.y ||
            target.x + target.width > area.x + area.width + 1 ||
            target.y + target.height > area.y + area.height + 1
        )
    ...
    finish() {
        ...
        if (!drag) return;
        const state = this.state();
        const region = drag.offers.find(
            (r) =>
                state.active &&
                state.x >= r.hit.x &&
                state.x < r.hit.x + r.hit.width &&
                state.y >= r.hit.y &&
                state.y < r.hit.y + r.hit.height &&
                (r.control
                    ? Boolean(state.modifiers & Clutter.ModifierType.CONTROL_MASK)
                    : !(state.modifiers & Clutter.ModifierType.CONTROL_MASK)),
        );
        this.cancel();
        ...
        this.pending = GLib.idle_add(GLib.PRIORITY_HIGH_IDLE, () => {
            ...
            try {
                ...
                this.bridge.send(drag.owner, { event: "error", message: error.message });
            }
    ...
    close(client) {
        if (this.drag?.owner === client) this.cancel();
    }
    forget(id) {
        this.saved.delete(id);
    }
