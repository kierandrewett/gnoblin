...
class Paste:
    def __init__(self, text):
        ...
        self.Target = Target
        self.gtk.gtk_clipboard_set_with_data.argtypes = [C.c_void_p, C.POINTER(Target), C.c_uint,
                                                        type(self.get_callback), type(self.clear_callback), C.c_void_p]
    ...
    def targets(self, _clipboard, atoms, *_args):
        self.pending = [a for a in (atoms or []) if a.name() not in {'TARGETS', 'MULTIPLE', 'TIMESTAMP', 'SAVE_TARGETS'}]
    ...
    def capture_next(self):
        ...
        if not self.own([('UTF8_STRING', 'UTF8_STRING', 8, self.text),
    ...
    def own(self, payload):
        ...
        self.entries = (self.Target * len(payload))(*[self.Target(row[0].encode(), 0, i) for i, row in enumerate(payload)])
        return self.gtk.gtk_clipboard_set_with_data(self.ptr, self.entries, len(payload),
                                                   self.get_callback, self.clear_callback, None)
