...
function readRecord(stream, cancel) {
    return new Promise((resolve, reject) =>
        stream.read_line_async(GLib.PRIORITY_DEFAULT, cancel, (input, result) => {
            try {
...
export class ClipboardPaste {
    ...
    async paste(text, checkFocus) {
        ...
        try {
            checkFocus();
            const helper = Gio.File.new_for_uri(import.meta.url)
                .get_parent()
                .get_child("clipboard-paste.py")
                .get_path();
            ...
            try {
                child?.get_stdin_pipe().close(null);
            } catch (_) {
                /* Already closed. */
            }
