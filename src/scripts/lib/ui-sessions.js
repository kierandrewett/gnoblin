...
export class UiSessions {
    ...
    command(client, record) {
        if (record.action === "watch") {
            ...
            for (const [name, owner] of this.owners) this.send(client, { event: "ui-state", name, state: owner.state });
            return;
        }
    ...
    reveal() {
        const requests = [...this.owners.values()]
            .map((owner) => owner.state)
            .filter(
                (state) =>
                    (state.visible || state.revealCompanions === true) &&
