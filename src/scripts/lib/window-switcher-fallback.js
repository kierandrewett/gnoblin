...
export class WindowSwitcherFallback {
    ...
    claim(client, record) {
        ...
        if (current.client !== this.client && current.client !== client)
            throw new Error("Switcher shortcut already owned");
        if (record.hold !== base.hold || record.modal === false)
            throw new Error("Switcher requires a modal modifier hold");
    ...
    key(key) {
        ...
        if (key === Clutter.KEY_Escape) {
            this.bridge.end("cancelled");
            return true;
        }
        if (key === Clutter.KEY_Return || key === Clutter.KEY_KP_Enter) {
            this.bridge.end("released");
            return true;
        }
        ...
        if (gesture.client === this.client || gesture.client.closed) this.commitPending();
            ...
            this.timer = GLib.timeout_add(GLib.PRIORITY_HIGH, 80, () => {
                this.timer = 0;
                this.commitPending();
                return GLib.SOURCE_REMOVE;
            });
