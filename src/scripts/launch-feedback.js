...
function normalise(value) {
    return String(value ?? "").toLowerCase().replace(/\.desktop$/, "").trim();
}
...
function loadStandardCursor(theme, size) {
    const roots = [GLib.build_filenamev([GLib.get_home_dir(), ".icons"]),
    ...
    for (const name of [...new Set([theme, "Adwaita"])]) {
        for (const root of roots) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                if (word(offset) === 0xfffd0002)
                    entries.push({size: word(offset + 4), offset: word(offset + 8)});
            }
            if (!entries.length) continue;
            const closest = entries.reduce((best, entry) => Math.abs(entry.size - size) < Math.abs(best - size) ? entry.size : best, entries[0].size);
            const frames = [];
            for (const entry of entries.filter(value => value.size === closest)) {
                ...
                if (!width || !height || width > 512 || height > 512 || pixels + width * height * 4 > bytes.length) continue;
                const image = St.ImageContent.new_with_preferred_size(width, height);
                image.set_bytes(global.stage.context.get_backend().get_cogl_context(),
                    new GLib.Bytes(bytes.slice(pixels, pixels + width * height * 4)),
                    Cogl.PixelFormat.BGRA_8888_PRE, width, height, width * 4);
                frames.push({image, width, height, hotX: word(offset + 24), hotY: word(offset + 28), delay: Math.max(16, word(offset + 32))});
            }
...
class LaunchFeedback {
    constructor() {
        ...
        this.nameId = Gio.bus_own_name(Gio.BusType.SESSION, NAME,
            Gio.BusNameOwnerFlags.NONE, null, null, () => this.clear());
    }
    ...
    Begin(token, application, timeout) {
        ...
        if (this.launches.size >= 64 && !this.launches.has(token))
            throw new Error("Too many pending launches");
    ...
    matches(window, hint) {
        if (!hint || window.skip_taskbar)
            return false;
        const app = Shell.WindowTracker.get_default().get_window_app(window);
        return [window.get_gtk_application_id(), window.get_wm_class(),
            window.get_wm_class_instance(), app?.get_id(), app?.get_name()]
    ...
    update() {
        ...
        for (const [token, launch] of this.launches) {
            const appeared = actors.some(actor => actor.is_mapped() &&
            ...
            if (now >= launch.deadline || appeared)
                this.launches.delete(token);
        }
        if (this.launches.size === 0) {
            this.hide();
