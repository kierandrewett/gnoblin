...
export const CAPABILITIES = Object.freeze(["screen-cast", "remote-desktop", "input-capture", "screenshot", "access"]);
...
export function validate(policy = DEFAULT_POLICY) {
    if (
        !table(policy) ||
        Object.keys(policy).some((key) => !["default", "rules"].includes(key)) ||
        !LEVELS.includes(policy.default ?? "default") ||
    ...
    if (policy.default === "allow") throw new Error("permissions.default cannot allow; use an explicit app rule");
    const names = new Set();
    ...
    for (const rule of rules) {
        if (
            ...
            Object.keys(rule).some(
                (key) => !["name", "match", "capabilities", "level", "monitors", "devices", "clipboard"].includes(key),
            ) ||
            typeof rule.name !== "string" ||
            !/^[a-zA-Z0-9_.-]{1,80}$/.test(rule.name) ||
            names.has(rule.name) ||
            !LEVELS.includes(rule.level) ||
            !Array.isArray(rule.capabilities) ||
            !rule.capabilities.length ||
            !rule.capabilities.every((cap) => CAPABILITIES.includes(cap)) ||
            typeof rule.match !== "string" ||
            !rule.match.length ||
            rule.match.length > 512
        )
            throw new Error("invalid permission rule: expected unique name, identity regex, capabilities and level");
        new RegExp(rule.match);
        names.add(rule.name);
        if (
            rule.monitors !== undefined &&
            (!Array.isArray(rule.monitors) ||
                !rule.monitors.length ||
        ...
        if (
            (rule.monitors !== undefined &&
                !rule.capabilities.some((cap) => ["screen-cast", "remote-desktop"].includes(cap))) ||
            ((rule.devices !== undefined || rule.clipboard !== undefined) &&
                !rule.capabilities.includes("remote-desktop"))
        )
...
export function evaluate(policy, capability, identity) {
    ...
    for (const rule of policy.rules) {
        ...
        Object.assign(result, {
            ...
            devices: (rule.devices ?? []).reduce(
                (mask, device) => mask | { keyboard: 1, pointer: 2, touchscreen: 4 }[device],
                0,
            ),
