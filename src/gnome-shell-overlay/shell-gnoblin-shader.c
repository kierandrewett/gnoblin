...
static gboolean compile_shader(GLuint shader, const char* source, GError** error) {
    GLint status;
    char log[4096] = {0};
    ...
    g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_DATA, "Shader compile failed: %s", log);
    return FALSE;
}
 ...
 */
gboolean shell_gnoblin_validate_shader(const char* source, GError** error) {
    GLuint vertex, fragment, program = 0;
    GLint status = FALSE;
    char log[4096] = {0};
    const char* version =
        epoxy_is_desktop_gl() ? "#version 120\n" : "#version 100\nprecision mediump float;\n";
    g_autofree char* vertex_source =
        g_strconcat(version,
                    "attribute vec4 position;\n"
                    "varying vec4 cogl_tex_coord_in[1]; varying vec4 cogl_color_in;\n"
                    "void main() { gl_Position=position; cogl_tex_coord_in[0]=position; "
                    "cogl_color_in=vec4(1.0); }",
                    NULL);
    g_autofree char* fragment_source =
        g_strconcat(version,
                    "varying vec4 cogl_tex_coord_in[1]; varying vec4 cogl_color_in;\n"
    ...
    if (!status) {
        ...
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_DATA, "Shader link failed: %s", log);
    }
out:
    if (program)
