...
static PendingCall* pending_call_new(CcGnoblinPanel* self, GtkWidget* widget) {
...
static ListCall* list_call_new(CcGnoblinPanel* self, guint serial) {
...
static void remove_rows(AdwPreferencesGroup* group, GList** rows) {
...
static void update_availability(CcGnoblinPanel* self) {
    g_autofree char* owner = self->proxy ? g_dbus_proxy_get_name_owner(self->proxy) : NULL;
...
static void call_done_cb(GObject* source, GAsyncResult* res, gpointer user_data) {
...
static AdwSwitchRow* find_feature_row(CcGnoblinPanel* self, const char* id) {
...
static void set_feature_row_active(AdwSwitchRow* row, gboolean enabled) {
    g_object_set_data(G_OBJECT(row), "gnoblin-feature-syncing", GINT_TO_POINTER(TRUE));
    adw_switch_row_set_active(row, enabled);
    g_object_set_data(G_OBJECT(row), "gnoblin-feature-syncing", NULL);
}

static void on_proxy_signal(GDBusProxy* proxy, const char* sender_name, const char* signal_name,
...
static void set_feature_done_cb(GObject* source, GAsyncResult* res, gpointer user_data) {
    ...
    ret = g_dbus_proxy_call_finish(G_DBUS_PROXY(source), res, &error);
    if (g_error_matches(error, G_IO_ERROR, G_IO_ERROR_CANCELLED) || self->disposed)
        return;
...
static void on_feature_row_activated(AdwSwitchRow* row, GParamSpec* pspec, gpointer user_data) {
    CcGnoblinPanel* self = CC_GNOBLIN_PANEL(user_data);
    const char* id;
    gboolean enabled;

    if (self->proxy == NULL || g_object_get_data(G_OBJECT(row), "gnoblin-feature-syncing") != NULL)
        return;
    ...
    g_dbus_proxy_call(self->proxy, "SetFeature", g_variant_new("(sb)", id, enabled),
                      G_DBUS_CALL_FLAGS_NONE, -1, self->cancellable, set_feature_done_cb,
                      pending_call_new(self, GTK_WIDGET(row)));
}

static void list_features_cb(GObject* source, GAsyncResult* res, gpointer user_data) {
    ...
    while (g_variant_iter_loop(iter, "(ssb)", &id, &summary, &enabled)) {
        ...
        g_object_set_data_full(G_OBJECT(row), "gnoblin-feature-id", g_strdup(id), g_free);
        g_signal_connect(row, "notify::active", G_CALLBACK(on_feature_row_activated), self);
...
static void reload_features(CcGnoblinPanel* self) {
    ...
    g_dbus_proxy_call(self->proxy, "ListFeatures", NULL, G_DBUS_CALL_FLAGS_NONE, -1,
                      self->cancellable, list_features_cb, call);
}

/* --- portal grants ------------------------------------------------------- */

static void revoke_grant_done_cb(GObject* source, GAsyncResult* res, gpointer user_data) {
    g_autofree PendingCall* call = user_data;
    g_autoptr(CcGnoblinPanel) self = g_steal_pointer(&call->self);
...
static void on_revoke_clicked(GtkButton* button, gpointer user_data) {
    ...
    g_dbus_proxy_call(self->proxy, "RevokePortalGrant", g_variant_new("(ss)", portal, id),
                      G_DBUS_CALL_FLAGS_NONE, -1, self->cancellable, revoke_grant_done_cb,
                      pending_call_new(self, GTK_WIDGET(button)));
}

static void list_grants_cb(GObject* source, GAsyncResult* res, gpointer user_data) {
    g_autofree ListCall* call = user_data;
    g_autoptr(CcGnoblinPanel) self = g_steal_pointer(&call->self);
    g_autoptr(GError) error = NULL;
    g_autoptr(GVariant) ret = NULL;
    ...
    while (g_variant_iter_loop(iter, "(&s&s&subb)", &id, &portal, &requester, &remote_devices,
                               &clipboard_enabled, &has_screen_streams)) {
...
static void reload_grants(CcGnoblinPanel* self) {
    ...
    g_dbus_proxy_call(self->proxy, "ListPortalGrants", NULL, G_DBUS_CALL_FLAGS_NONE, -1,
...
static void on_reload_clicked(GtkButton* button, gpointer user_data) {
    ...
    g_dbus_proxy_call(self->proxy, "Reload", NULL, G_DBUS_CALL_FLAGS_NONE, -1, self->cancellable,
                      call_done_cb, NULL);
}

/* --- proxy lifecycle ----------------------------------------------------- */

static void proxy_ready_cb(GObject* source, GAsyncResult* res, gpointer user_data) {
    ...
    if (proxy == NULL) {
        g_warning("gnoblin: could not connect to %s: %s", GNOBLIN_BUS_NAME,
    ...
    g_signal_connect_object(proxy, "notify::g-name-owner", G_CALLBACK(on_name_owner_changed), self,
                            0);
    g_signal_connect_object(proxy, "g-signal", G_CALLBACK(on_proxy_signal), self, 0);
    update_availability(self);
}

/* --- GObject boilerplate ------------------------------------------------- */

static void cc_gnoblin_panel_dispose(GObject* object) {
...
static void cc_gnoblin_panel_class_init(CcGnoblinPanelClass* klass) {
    ...
    gtk_widget_class_set_template_from_resource(
        widget_class, "/org/gnome/control-center/gnoblin/cc-gnoblin-panel.ui");
...
static void cc_gnoblin_panel_init(CcGnoblinPanel* self) {
    ...
    g_dbus_proxy_new_for_bus(G_BUS_TYPE_SESSION, G_DBUS_PROXY_FLAGS_NONE, NULL, GNOBLIN_BUS_NAME,
                             GNOBLIN_OBJECT_PATH, GNOBLIN_IFACE, self->cancellable, proxy_ready_cb,
                             g_object_ref(self));
}
