...
and dash so a bring-your-own layer-shell client — Quickshell, waybar, a
custom client — draws the UI instead), its login-manager entry and
gnoblin-specific systemd --user units (installing this does not affect any
other GNOME Shell session on the system — see org.gnoblin.Shell.target),
and gnoblinctl, the control CLI for the org.gnoblin.Shell D-Bus protocol
...
# Drop-in that pulls the shell target into the session. systemd-managed
# gnome-session ignores the .session RequiredComponents= line; the session's
# deps come from this gnome-session@gnoblin.target.d/ drop-in. Without it, login
# reaches a bare session with no compositor.
install -Dm644 %{SOURCE11} \
  %{buildroot}%{_userunitdir}/gnome-session@gnoblin.target.d/gnoblin.conf
...
%exclude %{_datadir}/gnome-shell/modes/gnoblin.json
...
%{_userunitdir}/gnome-session@gnoblin.target.d/gnoblin.conf

%changelog
## START: Generated by rpmautospec
* Thu Apr 16 2026 Petr Schindler <pschindl@redhat.com> - 49.6-1
- Update to 49.6

* Wed Mar 18 2026 Milan Crha <mcrha@redhat.com> - 49.5-1
- Update to 49.5
