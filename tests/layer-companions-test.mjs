...
const source = readFileSync(new URL('../src/scripts/lib/layer-companions.js', import.meta.url), 'utf8')
    .replace(/^import .*;\n/gm, '').replace('export class LayerCompanions', 'this.LayerCompanions = class LayerCompanions');
const signals = {connect() { return 1; }, disconnect() {}};
const parent = {
    children: [], get_children() { return this.children.slice(); },
    ...
    set_child_above_sibling(child, sibling) { this.place(child, sibling, true); },
    set_child_below_sibling(child, sibling) { this.place(child, sibling, false); }
};
const actor = name => ({...signals, visible: true, meta_window: {name, get_monitor() { return 0; }}, get_parent() { return parent; }});
const bar = actor('bar'), dock = actor('dock'), app = actor('fullscreen'), editor = actor('editor'), popup = actor('popup');
...
assert.deepEqual(parent.children, [app, editor, bar, dock, popup], 'Raise actual containers above the editor in their existing order');
layers.apply();
...
assert.deepEqual(parent.children, [app, editor, bar, dock, popup], 'Existing search companions remain below their overlay');
...
let request;
const sessions = new UiSessions(() => {}, {update(value) { request = value; }});
sessions.command({}, {action: 'state', name: 'customise', state: {
    visible: true, surface: 'editor', companions: ['bar', 'dock'], companionsAbove: true
}});
assert.deepEqual(request, [{surface: 'editor', companions: ['bar', 'dock'], companionsAbove: true}],
