...
import {Shortcuts, CommandShortcuts, ShortcutInput, validateShortcuts} from '../src/gnome-shell-overlay/js/ui/components/gnoblinConfig.js';
...
const settings = new Gio.Settings({schema_id: 'org.gnome.settings-daemon.plugins.media-keys.custom-keybinding', path});
...
assert(ok && JSON.stringify(argv) === JSON.stringify(entry.command), 'argv survives spaces, quotes and literal variables');
let changes = 0;
...
for (const invalid of [
    ...
    let rejected = false;
    try { manager.apply(invalid); } catch { rejected = true; }
...
manager.apply({});
assert(JSON.stringify(media.get_strv('custom-keybindings')) === JSON.stringify([`${base}user-owned/`]), 'remove only config-owned shortcut');
...
let nextAction = 100;
const grabs = [], releases = [], launched = [];
const display = {
    connect(signal, callback) { if (signal === 'overlay-key') { overlayReleased = callback; return 2; } activated = callback; return 1; },
    disconnect(id) { if (id === 2) overlayReleased = null; else activated = null; },
    grab_accelerator(binding, flags) {
        grabs.push({binding, flags});
        return binding === '<Alt>x' ? Meta.KeyBindingAction.NONE : nextAction++;
    },
    ungrab_accelerator(action) { releases.push(action); },
};
const commands = new CommandShortcuts(display, () => {}, argv => launched.push(argv));
commands.apply([entry]);
assert(grabs[0].flags === Meta.KeyBindingFlags.IGNORE_AUTOREPEAT, 'held shortcut ignores keyboard repeat at compositor');
activated(null, 100);
...
let conflict = false;
try { commands.apply([{...entry, binding: '<Alt>x'}]); } catch { conflict = true; }
...
function event(symbol, type = Clutter.EventType.KEY_PRESS) {
    return {type: () => type, get_key_symbol: () => symbol,
        copy: () => ({put: () => replayed.push(symbol)})};
}
input.begin('search');
assert(capture(stage, event(97)) === Clutter.EVENT_STOP, 'hold typing before popup focus');
capture(stage, event(Clutter.KEY_BackSpace));
assert(capture(stage, event(Clutter.KEY_Super_L, Clutter.EventType.KEY_RELEASE)) === Clutter.EVENT_PROPAGATE,
