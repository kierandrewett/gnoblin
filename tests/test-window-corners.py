...
for shape, opacity in ((0, 1), (40, 1), (120, 1), (0, 0.5)):
    configure({"radius": 0})
    ...
    try:
        ...
        if shape == 0:
            ...
            for action, keep in [("maximize", "keep-maximized"), ("fullscreen", "keep-fullscreen")]:
                ...
                assert not json.loads((root / "corner-frames.json").read_text())[0]["effect"], (
                    action + " did not suspend rounding"
                )
        ...
        time.sleep(0.2)
print(
    "PASS: native circular/smoothed corners, border, shadows, translucent contents, maximised/fullscreen policy, shaped/already-rounded bypass and removal"
)
