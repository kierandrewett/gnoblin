...
def state(): return json.loads(call('State')[0])
def wait(predicate):
...
try:
    for title in ['Focus Target', 'Focus Other']:
        ...
        apps.append(subprocess.Popen(['foot', '--title=' + title, 'sh', '-c', 'read line; printf "%s" "$line" > "$1"; sleep 30', 'sh', str(destination)],
    ...
    if expected:
        ...
        while time.monotonic() < end and not result.exists(): time.sleep(.05)
    ...
    subprocess.run(['wayland-scanner', 'client-header', protocol, str(config / 'xdg-activation-v1-client-protocol.h')], check=True)
    ...
    subprocess.run(['cc', str(root / 'tests/focus-transfer-client.c'), str(config / 'activation-protocol.c'),
        '-I' + str(config), '-o', str(binary), *flags], check=True)
    request = config / 'activation-request'
    received = config / 'activation-input'
    apps.append(subprocess.Popen([str(binary), str(request), str(received)], env=os.environ | {'GDK_BACKEND': 'wayland'}))
    ...
    if expected:
        ...
        while time.monotonic() < end and not received.exists(): time.sleep(.05)
    ...
    print('PASS: ' + ('stale app activation restores, changes workspace, and receives typing' if expected else 'GNOME retains its focus prevention'))
finally:
    for app in apps:
        app.terminate()
        app.wait()
