...
def evaluate(code):
    ok, value = bus.call_sync('org.gnome.Shell', '/org/gnome/Shell',
                              'org.gnome.Shell', 'Eval', GLib.Variant('(s)', (code,)),
...
with log_path.open('w') as log:
    proc = subprocess.Popen([os.environ.get('GNOBLIN_QS', 'qs'), '-p', str(fixture)],
...
global.__gnoblinLayerPerf.frameId = global.stage.connect('after-paint',
 () => global.__gnoblinLayerPerf.frames++); true''')
        ...
        result = evaluate('({workareas: global.__gnoblinLayerPerf.workareas, frames: global.__gnoblinLayerPerf.frames})')
        result.update(cpuPercent=round((cpu_ticks() - before) / os.sysconf('SC_CLK_TCK') / elapsed * 100, 2),
