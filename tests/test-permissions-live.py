...
backend = subprocess.Popen(
    [str(ROOT / "build/xdg-desktop-portal-gnome/src/xdg-desktop-portal-gnome"), "--replace"],
    stdout=backend_log,
    stderr=backend_log,
...
def handles():
    ...
    serial += 1
    return (
        f"/org/freedesktop/portal/desktop/request/{owner}/p{serial}",
...
try:
    for attempt in range(50):
        try:
            ...
            if backend.poll() is not None:
                raise RuntimeError("Portal backend exited")
            time.sleep(0.1)
    else:
        raise RuntimeError("Portal backend did not initialise ScreenCast")
    policy_rules = []
    for capability in ("screen-cast", "remote-desktop", "input-capture", "screenshot", "access"):
        ...
        if capability == "screen-cast":
            rule["monitors"] = ["primary"]
        if capability == "remote-desktop":
            rule["devices"] = ["keyboard", "pointer"]
    ...
    apply_policy(
        "deny",
        policy_rules
        + [{"name": "block-capture", "level": "deny", "match": app_match, "capabilities": ["screen-cast"]}],
    )
    ...
    assert (
        portal(
            "RemoteDesktop", "SelectDevices", "(oosa{sv})", (request, session, app, {"types": GLib.Variant("u", 3)})
        ).unpack()[0]
        == 0
    )
    ...
    result = portal(
        ...
        "CreateSession",
        "(oossa{sv})",
        (request, session, app, "", {"capabilities": GLib.Variant("u", 3)}),
    ).unpack()
    ...
    assert (
        portal(
            "RemoteDesktop", "SelectDevices", "(oosa{sv})", (request, session, app, {"types": GLib.Variant("u", 7)})
        ).unpack()[0]
        == 0
    )
    ...
    for capability, interface, method, signature in [
        ...
        apply_policy(
            "deny",
            policy_rules + [{"name": "block", "level": "deny", "match": app_match, "capabilities": [capability]}],
        )
        request, session = handles()
        values = (request, app, "", {})
        if capability == "access":
            values = (request, app, "", "Test", "Test", "Test", {})
        if capability == "input-capture":
            values = (request, session, app, "", {"capabilities": GLib.Variant("u", 3)})
    ...
    apply_policy(
        "deny", policy_rules + [{"name": "prompt", "level": "ask", "match": app_match, "capabilities": ["screen-cast"]}]
    )
    request, session = handles()
    assert portal("ScreenCast", "CreateSession", "(oosa{sv})", (request, session, app, {})).unpack()[0] == 0
    assert (
        portal(
            ...
            "SelectSources",
            "(oosa{sv})",
            (request, session, app, {"types": GLib.Variant("u", 1), "restore_data": restore}),
        ).unpack()[0]
        == 0
    )
    responses = []
    ...
    bus.call(
        ...
        "Start",
        GLib.Variant("(oossa{sv})", (request, session, app, "", {})),
        None,
        Gio.DBusCallFlags.NONE,
        6000,
        None,
    ...
    direct = Gio.DBusConnection.new_for_address_sync(
    ...
    request, _ = handles()
    assert (
        portal("Access", "AccessDialog", "(osssssa{sv})", (request, "", "", "Test", "Test", "Test", {})).unpack()[0]
        == 0
    )
    apply_policy("ask", policy_rules)
    assert ctl("permissions", "list")["policy"]["default"] == "ask"
    print(
        "PASS: real portal unattended ScreenCast, RemoteDesktop, InputCapture, Screenshot, Access; deny overrides restore data"
    )
finally:
    backend.terminate()
