...
evaluate(f'imports.gi.GIRepository.Repository.dup_default().prepend_search_path({json.dumps(probe)});imports.gi.GIRepository.Repository.dup_default().prepend_library_path({json.dumps(probe)});global.dragFrameProbe=imports.gi.FrameProbe2;true')
try:
    for title in ['RustDesk', 'Nested Ghostty']:
        evaluate(f'global.dragWindow=global.get_window_actors().find(a=>a.meta_window.title==={json.dumps(title)}).meta_window;global.dragWindow.unminimize();global.dragWindow.activate(global.get_current_time());global.dragWindow.move_frame(false,200,100);true')
        time.sleep(.7)
        ...
        evaluate(f'global.dragFrameProbe.arm(global.stage,{json.dumps(str(incremental))});global.stage.queue_redraw_with_clip(new imports.gi.Mtk.Rectangle({{x:200,y:100,width:1,height:1}}));true')
        time.sleep(.3)
