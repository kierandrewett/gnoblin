...
def configure(value):
    ...
    temp.write_text(json.dumps(value)); temp.replace(config); time.sleep(.4)
configure({})
source = (Path(__file__).resolve().parents[1] / 'tests/window-corners-native.js').read_text()
source = source.replace('corners?.effect', 'borders?.widget').replace('_action, ...corners}', '_action, _shadow, _animation, ...borders}').replace('},corners}', '},borders}')
source = source.replace("match:{type:'window'},borders", "match:{type:'window'},borders,corners:{radius:borders.radius ?? 0,smoothing:borders.smoothing ?? 0,mode:'force','shadow-animation':_animation ?? {duration:0},shadow:_shadow ?? (borders.radius ? {blur:24,spread:0,opacity:0.35} : false)}")
...
proc = subprocess.Popen(['qs','-p',str(qml)], stdout=subprocess.DEVNULL, stderr=(root/'border-client.log').open('w'), env={**os.environ, 'QT_WAYLAND_DISABLE_WINDOWDECORATION':'1'})
def capture():
...
try:
    for _ in range(30):
        ...
        if frames.exists() and json.loads(frames.read_text()): break
    time.sleep(.6)
    configure({})
    ...
    assert sum(after.getpixel((x+w//2,y-1))) < sum(before.getpixel((x+w//2,y-1)))-30, 'outer ring missing'
    ...
    point = (x+w//2,y+h+1)
    configure({**rule,'_shadow':[broad]}); broad_pixel = sum(capture().getpixel(point))
    configure({**rule,'_shadow':[contact]}); contact_pixel = sum(capture().getpixel(point))
    configure({**rule,'_shadow':[broad,contact]}); layered_pixel = sum(capture().getpixel(point))
    assert layered_pixel < min(broad_pixel,contact_pixel)-4, ('layers not combined',broad_pixel,contact_pixel,layered_pixel)
    ...
    assert clear_pixel > middle_pixel+4 and middle_pixel > dark_pixel+4, ('shadow did not fade',clear_pixel,middle_pixel,dark_pixel)
