...
test('opaque buffers use compositor metadata without GPU readback', () => {
    ...
    vm.runInContext(source.slice(source.indexOf('const visibleFrames ='), source.indexOf('const declarations =')), context);
    let reads = 0;
    const rectangle = {x: 10, y: 20, width: 800, height: 600};
    const actor = {width: 800, height: 600, mapped: true, opacity: 255,
        get_resource_scale: () => 1,
        get_texture: () => ({get_texture: () => ({}), is_opaque: () => true}),
        get_image: () => {reads++; throw new Error('Unexpected GPU readback');},
