...
def measure(name):
    ...
    rows.append(
        {
            "phase": name,
            "cpuPercent": round((after["ticks"] - before["ticks"]) / os.sysconf("SC_CLK_TCK") / elapsed * 100, 2),
            "clientDraws": draws - initial_draws,
            **{k: v for k, v in after.items() if k != "ticks"},
        }
    )
...
assert next(row for row in rows if row["phase"] == "partially-covered-animation")["clientDraws"] > 30, (
    "Partially visible animation stopped"
)
if os.environ.get("GNOBLIN_BENCH_REQUIRE_OCCLUSION") == "1":
    assert next(row for row in rows if row["phase"] == "covered-animation")["clientDraws"] < 10, (
        "Covered animation kept rendering"
    )
