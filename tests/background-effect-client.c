...
static void configure(void* data, struct zwlr_layer_surface_v1* layer, uint32_t serial, uint32_t w,
                      uint32_t h) {
...
static struct wl_surface* layer(int width, int height, int background) {
    ...
    struct zwlr_layer_surface_v1* role = zwlr_layer_shell_v1_get_layer_surface(
        layers, surface, NULL,
    ...
    zwlr_layer_surface_v1_set_anchor(role, ZWLR_LAYER_SURFACE_V1_ANCHOR_TOP |
                                               ZWLR_LAYER_SURFACE_V1_ANCHOR_LEFT);
    zwlr_layer_surface_v1_set_size(role, width, height);
    if (!background)
        zwlr_layer_surface_v1_set_margin(role, 128, 0, 0, 128);
...
int main(int argc, char** argv) {
    ...
    if (!strcmp(argv[1], "duplicate") || !strcmp(argv[1], "dead")) {
        ...
        assert(interface == (!strcmp(argv[1], "duplicate")
    ...
    while (fgets(line, sizeof line, stdin)) {
        if (!strncmp(line, "set", 3))
            ...
            ext_background_effect_surface_v1_set_blur_region(effect, empty);
            wl_region_destroy(empty);
        } else if (!strncmp(line, "commit", 6))
            wl_surface_commit(surface);
        else if (!strncmp(line, "destroy", 7)) {
            ext_background_effect_surface_v1_destroy(effect);
            effect = NULL;
        } else if (!strncmp(line, "recreate", 8))
            effect = ext_background_effect_manager_v1_get_background_effect(manager, surface);
        else if (!strncmp(line, "manager", 7)) {
            ...
            struct ext_background_effect_surface_v1* child_effect =
                ext_background_effect_manager_v1_get_background_effect(manager, child);
