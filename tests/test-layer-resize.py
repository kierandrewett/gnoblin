...
with tempfile.TemporaryDirectory(prefix='gnoblin-layer-resize-') as directory:
    ...
    for mode, xml, filename in [('client-header', layer, 'wlr-layer-shell-unstable-v1-client-protocol.h'),
    ...
    subprocess.run(['cc', '-I' + str(build), str(repo / 'tests/layer-shell-resize-client.c'),
                    str(build / 'layer.c'), str(build / 'xdg.c'), *flags, '-o', str(build / 'client')], check=True)
    for anchor in ['bottom', 'right', 'top']:
        proc = subprocess.Popen([str(build / 'client'), anchor], stdin=subprocess.PIPE, stdout=subprocess.PIPE, text=True)
        boxes = []
        try:
            ...
            if anchor == 'bottom': assert boxes[0][3] == boxes[2][3], boxes
            if anchor == 'right': assert boxes[0][2] == boxes[2][2], boxes
            if anchor == 'top': assert boxes[2][0] == 50 and boxes[2][1] == boxes[0][1], boxes
