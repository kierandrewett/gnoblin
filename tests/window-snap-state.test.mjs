...
function fixture(flags = 0) {
    ...
    const source = readFileSync(new URL('../src/scripts/lib/window-snap.js', import.meta.url), 'utf8')
        .replace(/^import .*;\n/gm, '').replace('export class WindowSnap', 'class WindowSnap');
    const area = {x: 0, y: 32, width: 1280, height: 768};
    const context = vm.createContext({Meta: {MaximizeFlags: {BOTH: 3}},
    ...
    snap.saved = new Map(); snap.bridge = {eligible: () => true};
    const window = {get_stable_sequence: () => 1, get_maximize_flags: () => flags,
        unmaximize: () => {flags = 0; calls.push('restore');},
        maximize: () => {flags = 3; calls.push('maximize');},
        can_minimize: () => true, can_maximize: () => true,
        minimize: () => calls.push('minimize'), allows_move: () => true,
        allows_resize: () => true, is_fullscreen: () => false,
...
test('maximised and native tiled windows restore before minimising', () => {
    for (const flags of [1, 2, 3]) {
        ...
        snap.restoreOrMinimize(window); snap.restoreOrMinimize(window);
