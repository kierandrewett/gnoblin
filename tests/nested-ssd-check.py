...
evaluate('global.nestedPointer ??= global.stage.context.get_backend().get_default_seat().create_virtual_device(imports.gi.Clutter.InputDeviceType.POINTER_DEVICE);global.nestedKeys ??= global.stage.context.get_backend().get_default_seat().create_virtual_device(imports.gi.Clutter.InputDeviceType.KEYBOARD_DEVICE);true')

def input_event(kind, **data):
    if kind == 'move':
        evaluate(f'global.nestedPointer.notify_absolute_motion(imports.gi.GLib.get_monotonic_time(),{data["x"]},{data["y"]});true')
    elif kind == 'button':
        evaluate(f'global.nestedPointer.notify_button(imports.gi.GLib.get_monotonic_time(),{data.get("button",1)},{1 if data["down"] else 0});true')
    elif kind == 'key':
        evaluate(f'global.nestedKeys.notify_key(imports.gi.GLib.get_monotonic_time(),{data["code"]},{1 if data["down"] else 0});true')
    time.sleep(.06)

def inspect():
    return evaluate('(()=>{let w=global.get_window_actors().find(a=>a.meta_window.title==="SSD fixture").meta_window,f=w.get_frame_rect();return {frame:[f.x,f.y,f.width,f.height],minimized:w.minimized,maximized:w.maximized_horizontally&&w.maximized_vertically,layout:imports.gi.Meta.gnoblin_window_frame_get(w).recursiveUnpack()};})()')

if __name__ == '__main__':
    evaluate('global.get_window_actors().find(a=>a.meta_window.title==="SSD fixture").meta_window.activate(global.get_current_time());true')
    time.sleep(.5)
    state = inspect()
    ...
    if renderer == 'bingux':
        ...
        input_event('move', x=x + control[1] + control[3] // 2,
                    y=y + control[2] + control[4] // 2)
        time.sleep(.04)
        ...
        crop = (x + control[1], y + control[2],
                x + control[1] + control[3], y + control[2] + control[4])
        changed = sum(a != b for a, b in zip(before.crop(crop).getdata(),
                                              after.crop(crop).getdata()))
        mid_changed = sum(a != b for a, b in zip(before.crop(crop).getdata(),
                                                  mid.crop(crop).getdata()))
        settled_changed = sum(a != b for a, b in zip(mid.crop(crop).getdata(),
                                                      after.crop(crop).getdata()))
        hovered = evaluate('imports.gi.Meta.gnoblin_window_frame_get(global.get_window_actors().find(a=>a.meta_window.title==="SSD fixture").meta_window).recursiveUnpack().presentation.hover')
        ...
        assert mid_changed and settled_changed, ('Bingux control hover did not animate',
                                                  mid_changed, settled_changed)
        ...
        evaluate('global.get_window_actors().find(a=>a.meta_window.title==="Nested Ghostty").meta_window.focus(global.get_current_time());true')
        time.sleep(.07)
        ...
        assert focused_color != middle_color != unfocused_color, ('Bingux focus color snapped', focused_color, middle_color, unfocused_color)
        evaluate('global.get_window_actors().find(a=>a.meta_window.title==="SSD fixture").meta_window.activate(global.get_current_time());true')
        time.sleep(.3)
