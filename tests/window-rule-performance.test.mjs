...
function loadConfig(CountingRegExp) {
    ...
    const context = vm.createContext({
        ...
        Corners: {
            borderDefaults: {}, defaults: {}, merge: (previous, next) => ({...previous, ...next}),
            validate() {}, validateBorders() {},
        },
        Gio: {SettingsSchemaSource: {get_default() {return {lookup() {return null;}};}}},
        GLib: {}, Meta: {}, Clutter: {},
    });
    vm.runInContext(`${source}\nglobalThis.windowEffects = windowEffects; globalThis.parseDocument = parseDocument;`, context);
...
test('window rule regular expressions compile once and survive configuration replacement', () => {
    ...
    const rules = Array.from({length: 6}, (_, index) =>
        ({match: {title: `^window-${index}$`}, opacity: 0.9}));
    ...
    assert.throws(() => parseDocument({'window-rules': [{match: {title: '['}}]}),
