...
echo "== focused-app widget must open the shared app context menu =="
GNOBLIN_PREFIX="$PREFIX" python3 "$ROOT/tests/layer-shell/topbar-focused-app-menu-test.py" 2>&1 \
  | tee /tmp/gnoblin-topbar-focused-app-menu-test.log
exit "${PIPESTATUS[0]}"
