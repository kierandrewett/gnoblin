...
echo "== topbar must live-reload [topbar] action commands =="
GNOBLIN_PREFIX="$PREFIX" python3 "$ROOT/tests/layer-shell/topbar-live-commands-test.py" 2>&1 \
  | tee /tmp/gnoblin-topbar-live-commands-test.log
exit "${PIPESTATUS[0]}"
