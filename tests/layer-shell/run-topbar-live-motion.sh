...
echo "== topbar must live-reload Slint motion scale =="
GNOBLIN_PREFIX="$PREFIX" python3 "$ROOT/tests/layer-shell/topbar-live-motion-test.py" 2>&1 \
  | tee /tmp/gnoblin-topbar-live-motion-test.log
exit "${PIPESTATUS[0]}"
