#!/usr/bin/env bash
# Regression: keyboard tile-cycling (`tile left/right/up/down`) + `[appearance]
# gaps`. Drives a headless gnoblin-shell via the harness, tiles a window, and
# pixel-checks the cycle + gap. Needs a dev build + grim + foot + PIL.
set -uo pipefail
...
echo "== keyboard tile-cycling + gaps (headless) =="
GNOBLIN_PREFIX="$PREFIX" python3 "$ROOT/tests/layer-shell/tiling-test.py" 2>&1 \
