...
{
  fprintf (stderr, "layer-move-focus-client: %s\n", message);
  exit (2);
}
...
create_anonymous_file (size_t size)
{
  ...
  snprintf (path, sizeof path, "%s/gnoblin-layer-move-focus-XXXXXX",
            runtime_dir);
...
map_surface (struct app *app)
{
  ...
  app->buffer = create_buffer (app, 160, 80, 0xff34d399);
  if (!app->buffer)
                ...
                uint32_t           state)
{
  ...
  app->button_count++;
  fprintf (stdout, "BUTTON %u\n", app->button_count);
  fflush (stdout);

  if (app->button_count == 1 && !app->moved)
    {
      app->moved = true;
      zwlr_layer_surface_v1_set_margin (app->layer_surface, 220, 0, 0, 0);
      wl_surface_commit (app->surface);
      wl_display_flush (app->display);
      fprintf (stdout, "MOVED\n");
      fflush (stdout);
    }
}
      ...
      char **argv)
{
  ...
  app.layer_surface =
    zwlr_layer_shell_v1_get_layer_surface (app.layer_shell,
                                           ...
                                           "gnoblin-layer-move-focus");
  zwlr_layer_surface_v1_add_listener (app.layer_surface,
                                      &layer_surface_listener,
                                      &app);
  zwlr_layer_surface_v1_set_size (app.layer_surface, 160, 80);
