...
for (const [Effect, config] of [[WindowCorners, defaults], [WindowBorders, borderDefaults]]) {
    let texture = null;
    let captures = 0;
    const rect = {x: 0, y: 0, width: 400, height: 300};
    const surface = {width: 400, height: 300, get_effects: () => []};
    const actor = {
        mapped: true, opacity: 255,
        meta_window: {
            get_frame_rect: () => rect,
        ...
        get_effects: () => [],
        get_children: () => [],
        get_image: () => { captures++; return null; },
    };
    const instance = {actor, surface, toolkit: {}, remove() {}};
    Effect.prototype.update.call(instance, config);
    if (captures !== 0)
        throw new Error(`${Effect.name}: captured a mapped window without a buffer`);
    texture = {};
    actor.opacity = 128;
    Effect.prototype.update.call(instance, config);
    if (captures !== 0)
        throw new Error(`${Effect.name}: sampled the opening fade`);
    actor.opacity = 255;
    Effect.prototype.update.call(instance, config);
    if (captures !== 1)
        throw new Error(`${Effect.name}: did not retry when the buffer arrived`);
    texture = null;
    Effect.prototype.update.call(instance, config);
    if (captures !== 1)
        throw new Error(`${Effect.name}: captured after the buffer was removed`);
}
