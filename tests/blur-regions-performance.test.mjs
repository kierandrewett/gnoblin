...
const source = readFileSync(process.env.GNOBLIN_BLUR_REGIONS_SOURCE ??
    new URL('../src/scripts/lib/blur-regions.js', import.meta.url), 'utf8')
    .replace(/^import .*;\n/gm, '')
...
const context = vm.createContext({
    ...
    global: {
        get_window_actors() { scans++; return actors; },
...
function actor(pid, namespace = 'panel', monitor = 0) {
    const effect = {
        region: null,
        set_region(...value) { writes++; this.region = value; },
        clear_region() { writes++; this.region = null; },
    };
    const result = {
        effect,
