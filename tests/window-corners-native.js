...
export default function(api) {
    ...
    let pending = 0;
    const dump = () => {
        const rows = global.get_window_actors().filter(a => a.meta_window.title === 'Corner fixture').map(a => {
        ...
        Gio.File.new_for_path(GLib.get_user_config_dir()+'/gnoblin/corner-frames.json').replace_contents(JSON.stringify(rows),null,false,0,null);
    };
    const later = () => {
        if (pending) GLib.source_remove(pending);
        pending = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 150, () => {pending=0;dump();return GLib.SOURCE_REMOVE;});
    };
    const apply = () => {
    ...
    monitor.connect('changed',(_m,f,other) => {if(f?.equal(file)||other?.equal(file))apply();});
    apply();
    const id = global.window_manager.connect('map',later);
    api._disposers.push(() => {monitor.cancel();if(pending)GLib.source_remove(pending);global.window_manager.disconnect(id);});
}
