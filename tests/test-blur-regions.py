...
try:
    ...
    time.sleep(0.5)  # Let screencopy repair damage settle before measuring.
    before = state()
    time.sleep(1.5)
    after = state()
    ...
    delta = ImageStat.Stat(
        ImageChops.difference(red.crop((800, 200, 1120, 520)), reference.crop((800, 200, 1120, 520)))
    )
    assert max(delta.mean) < 1.5, delta.mean
    before_move = state()
    ipc("move", 700)
    ...
    assert max(delta.mean) < 1.5, delta.mean
    print(
        "PASS: cropped pixels, damage expansion, cache reuse outside damage, native region transport and movement",
        initial,
        bounded,
        after,
        flush=True,
