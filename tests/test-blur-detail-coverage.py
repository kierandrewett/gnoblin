...
""".replace("STEAM_ICON", icons["STEAM_ICON"]).replace("LOCALSEND_ICON", icons["LOCALSEND_ICON"])
)
if os.environ.get("GNOBLIN_TEST_STANDARD_BLUR") == "1":
    qml.write_text(
        ...
        .replace(
            "id: panel;",
            "id: panel; Native.BackgroundEffect { id: standardEffect; target: panel; radius: panel.radius }",
        )
