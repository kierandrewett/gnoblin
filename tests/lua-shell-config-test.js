...
`,
    ...
    config.start();
    assert(
        current["minimize-duration"] === 120 && current["minimize-animation"] === "none",
...
`,
    ...
    Gio.File.new_for_path(`${modulePath}.tmp`).move(
        Gio.File.new_for_path(modulePath),
    ...
    settle();
    assert(
        current["minimize-animation"] === "fade" && current["window-rules"].length === 0,
    ...
    settle();
    assert(updates === before && current["minimize-duration"] === 90, "removed dependencies stop triggering reloads");
    ...
    GLib.file_set_contents(`${directory}/parts/nested/90-extra.lua`, `return {shell = {['minimize-duration'] = 45}}`);
    settle();
    ...
    GLib.file_set_contents(`${external}/conf/motion.lua`, `return {shell = {['minimize-duration'] = 55}}`);
    ...
    Gio.File.new_for_path(`${external}/conf`).move(
        Gio.File.new_for_path(`${external}/previous`),
        Gio.FileCopyFlags.NONE,
        null,
        null,
    );
    GLib.mkdir_with_parents(`${external}/conf`, 0o700);
    GLib.file_set_contents(`${external}/conf/motion.lua`, `return {shell = {['minimize-duration'] = 65}}`);
    settle();
    assert(current["minimize-duration"] === 65, "replacing an external directory reloads the glob");
    GLib.file_set_contents(`${external}/conf/motion.lua`, `return {shell = {['minimize-duration'] = 85}}`);
    settle();
    ...
    config.destroy();
    for (const file of [path, modulePath, nested, `${modulePath}.tmp`]) GLib.unlink(file);
    GLib.unlink(`${directory}/parts/nested/90-extra.lua`);
