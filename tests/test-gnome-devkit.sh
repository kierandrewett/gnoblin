...
grep -q 'org.gnoblin.Shell owned' <<<"$out" || {
    echo "  FAIL: nested gnoblin did not come up"
    rc=1
}
grep -q 'PING:pong' <<<"$out" || {
    echo "  FAIL: gnoblinctl ping did not reach org.gnoblin.Shell in the devkit env"
    rc=1
}
grep -q 'VER:.*gnoblin' <<<"$out" || {
    echo "  FAIL: gnoblinctl version wrong in the devkit env"
    rc=1
}
...
if [ "$rc" = 0 ]; then
    ...
    echo ">> RESULT: FAIL. default output tail:"
    tail -n 20 <<<"$out"
    echo ">> unsafe output tail:"
    tail -n 20 <<<"$unsafe_out"
fi
exit "$rc"
