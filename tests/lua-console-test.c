...
int main(void) {
    ...
    g_autoptr(GVariant) rich =
        run("tree = {a = 42, nested = {true, false}}; tree.self = tree; setmetatable(tree, "
            "{__index = function() error('must not execute') end}); return tree");
    ...
    g_autoptr(GVariant) properties =
        g_variant_lookup_value(inspection, "details", G_VARIANT_TYPE("aa{sv}"));
    ...
    for (gsize i = 0; i < g_variant_n_children(properties); i++) {
        ...
        if (!strcmp(name, "[[Metatable]]"))
            metatable = TRUE;
        if (!strcmp(name, "\"self\"")) {
            g_autoptr(GVariant) value =
                g_variant_lookup_value(row, "value", G_VARIANT_TYPE_VARDICT);
            int child = 0;
