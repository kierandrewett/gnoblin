...
import {borderDefaults, validateBorders, bordersEnabled} from '../src/gnome-shell-overlay/js/ui/components/gnoblinCornerGeometry.js';
import {parseDocument, windowEffects} from '../src/gnome-shell-overlay/js/ui/components/gnoblinConfig.js';
function assert(ok, message) { if (!ok) throw new Error(message); }
for (const value of [null, [], {'inner-width': -1}, {'outer-width': 41}, {'inner-width': NaN}, {'outer-color': 'black'}, {radius: 201}, {typo: true}]) {
    let rejected = false; try { validateBorders(value); } catch (_) { rejected = true; }
    assert(rejected, JSON.stringify(value));
}
const rules = parseDocument({'window-rules': [
...
assert(result['inner-width'] === 1 && result['inner-color'] === '#505050ff' && result['outer-color'] === '#00000080', 'cascade');
...
for (const shadow of [[], Array(5).fill({}), [{blur:-1}], [false], [{opacity:2}]]) {
    let rejected=false; try {validate({shadow});} catch (_) {rejected=true;} assert(rejected,'bad shadow stack');
}
...
assert(merge({shadow:{blur:12,opacity:.5}}, {shadow:{blur:8}}).shadow.opacity === .5,'legacy table merges');
...
const bodyLine = (size, inset, alpha = 255) => Array.from({length:size}, (_, i) =>
...
assert(Geometry.detectEdge(Array.from({length:200},(_,i)=>Math.min(255,i*4))) === null, 'do not crop a soft gradient');
...
for (const animation of [false,[],{duration:-1},{duration:2001},{easing:'bounce'},{speed:2}]) {
    let rejected=false;try {Geometry.validate({'shadow-animation':animation});} catch (_) {rejected=true;}
    assert(rejected,'invalid shadow animation '+JSON.stringify(animation));
}
assert(Geometry.merge({'shadow-animation':{duration:200,easing:'linear'}},{'shadow-animation':{duration:300}})['shadow-animation'].easing === 'linear','merge animation fields');
