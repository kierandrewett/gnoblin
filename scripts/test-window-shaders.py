...
def write_shader(colour):
    ...
    temporary.write_text('uniform float strength;\nvec4 gnoblin_effect(vec4 color, vec2 uv) {'
...
try:
    ...
    print('PASS: shader pixels, alpha, blur composition, atomic reload, invalid/missing file recovery, uniforms and rule removal')
finally:
    proc.terminate()
    proc.wait(timeout=5)
