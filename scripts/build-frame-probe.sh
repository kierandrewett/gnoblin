...
api="$($task_root/scripts/gnome-versions.py get mutter api)"
read -r -a probe_flags <<<"$(pkg-config --cflags --libs "mutter-clutter-$api" gdk-pixbuf-2.0)"
probe_namespace="${FRAME_PROBE_NAMESPACE:-FrameProbe}"
probe_library="frame-probe-$probe_namespace"
gcc -shared -fPIC "$task_root/tests/frame-probe/probe.c" -o "$probe_output/lib$probe_library.so" "${probe_flags[@]}" -L"${GNOBLIN_PREFIX:-$task_root/install}/lib64" "-lmutter-$api"
LD_LIBRARY_PATH="${GNOBLIN_PREFIX:-$task_root/install}/lib64:${GNOBLIN_PREFIX:-$task_root/install}/lib64/mutter-$api${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}" \
    ...
    --include-uninstalled="$task_root/build/mutter/clutter/clutter/Clutter-$api.gir" \
    --add-include-path="$task_root/build/mutter/cogl/cogl" --add-include-path="$task_root/build/mutter/mtk/mtk" \
    --pkg="mutter-clutter-$api" --pkg=gdk-pixbuf-2.0 \
