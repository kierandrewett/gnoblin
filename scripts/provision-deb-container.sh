...
case "$ID:$VERSION_ID" in
    ...
    ubuntu:24.04)
        bundle=true
        rust_packages=(rustc-1.85 cargo-1.85)
        ;;
    debian:13)
        bundle=true
        rust_packages=(rustc cargo python3-legacy-cgi)
        ;;
    *)
...
apt-get install -y --no-install-recommends "${rust_packages[@]}" \
    python3-venv clang llvm cbindgen libreadline-dev zip zlib1g-dev valac \
    libheif-dev libjxl-dev libfontconfig-dev libevdev-dev hwdata \
...
python3 -m venv /opt/gnoblin-build-tools
