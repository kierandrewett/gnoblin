...
DBUS_CONF="$(python3 "$ROOT/scripts/devkit_dbus.py" "$DK" "$ROOT")" || {
    rm -rf "$DK"
    exit 1
}
DBUS_PID_FILE="$DK/dbus.pid"
DBUS_SESSION_BUS_ADDRESS="$(dbus-daemon --config-file="$DBUS_CONF" --print-address --fork --print-pid=3 3>"$DBUS_PID_FILE")" || {
    rm -rf "$DK"
    exit 1
}
...
if ! gdbus wait --session --timeout 30 org.gnoblin.Shell; then
    echo "!! gnoblin did not come up:" >&2
    tail -n 30 "$DK/shell.log" >&2
    exit 1
fi
...
[ -n "$TERM_BIN" ] || {
    echo "no terminal found (install foot/kitty/alacritty, or pass one)"
    exit 1
}
