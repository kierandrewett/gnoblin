...
cleanup() {
  ...
  [ -f "$SHELL_LOG" ] && gnoblin_publish_log "$SHELL_LOG" hot-reload-last.log 2>/dev/null || true
  rm -rf "$DK"
}
...
'
rc=$?
[ "$rc" = 0 ] && echo ">> RESULT: PASS (live extension hot-reload)" || echo ">> RESULT: FAIL (rc=$rc). log -> $LAST_LOG"
exit "$rc"
