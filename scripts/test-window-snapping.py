...
for name in ['SnapAssist.qml', 'SnapLayouts.js', 'import-snap-layouts.py', 'ShortcutSession.qml', 'ShellPopup.qml', 'Theme.qml', 'CompositorEnvironment.qml']:
...
if app_env['GDK_BACKEND'] == 'x11':
    ...
    app_env.update({key: value for key, value in json.loads((config / 'snap-display.json').read_text()).items() if value})
...
''')
    ...
    print('PASS: actual title-bar drag, picker drop, focus, restored size, keyboard selection, Escape, and Ctrl regions', flush=True)
finally:
    app.terminate()
    qs.terminate()
    app.wait()
