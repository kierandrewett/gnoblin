...
if not os.environ.get('WAYLAND_DISPLAY', '').startswith('gnoblin-gs-'):
    assert theme.resolve() == (Path.home() / '.local/share/icons' / THEME).resolve(), 'Install the theme before testing it'
...
class Frame(C.Structure):
    _fields_ = [('surface', C.c_void_p), ('size', C.c_int), ('delay', C.c_int),
                ('hotspotX', C.c_int), ('hotspotY', C.c_int)]
            ...
            for shape in [name, *info['aliases']]:
                ...
                try:
                    for i in range(count.value):
                        ...
                        assert (frame.hotspotX, frame.hotspotY) == tuple(math.floor(v * size / 24 + .5) for v in info['hotspot']), (size, shape)
                        assert frame.delay == max(1, info['duration']), (shape, frame.delay)
                        cairo.cairo_surface_flush(frame.surface)
                        pixels = C.string_at(cairo.cairo_image_surface_get_data(frame.surface),
