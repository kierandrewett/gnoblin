...
def crop_sheet(sheet, rectangle, bounds):
    ...
    result = ET.Element(SVG + 'svg', {'width': '24', 'height': '24',
        'viewBox': ' '.join(f'{v:g}' for v in rectangle), 'version': '1.1'})
...
def build(output, fallback):
    ...
    assert hashlib.sha256(svg.read_bytes()).hexdigest() == '3da7cbdaacb51ba90e6b7f903792f2ca58ca0ac3e01ba69c0cac248e397ba500'
    metadata = json.loads((SOURCE / 'cursors.json').read_text())
    sheet = ET.parse(svg).getroot()
    slices = {e.get('id'): tuple(float(e.get(k)) for k in ('x', 'y', 'width', 'height'))
        for layer in sheet if layer.get('id') == 'layer2'
        for e in layer if e.tag == SVG + 'rect'}
    ...
    with tempfile.TemporaryDirectory(prefix='adwaita-vector-', dir=output.parent) as temporary:
        ...
        bounds = {row[0]: tuple(map(float, row[1:])) for row in csv.reader(queried.stdout.splitlines()) if len(row) == 5}
        source = work / 'source'
        source.mkdir()
        (source / 'manifest.hl').write_text(f'name = {THEME}\ndescription = GNOME Adwaita 49 vector cursors, packaged by Gnoblin\nversion = 49.0\ncursors_directory = hyprcursors\n')
        ...
        subprocess.run(['hyprcursor-util', '--create', str(source), '--output', str(work)],
